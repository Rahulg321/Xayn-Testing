...
let testCategory = "Multi Document Topic Segregation";

let prompt =
  "Ability to segregate documents based on topics when the documents are in different languages. Do this for the attached documents";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  beforeAll(async () => {
    ...
    let documents = [
      {
        path: "./src/documents/french-legal-text.txt",
        name: "french-legal-text.txt",
      },
      {
        path: "./src/documents/spanish-nuances.txt",
        name: "spanish-nuances.txt",
      },
      {
        path: "./src/documents/german-nuances.txt",
        name: "german-nuances.txt",
      },
    ];
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      prompt,
      testCategory,
      testName: "Multi Document Topic Segregation",
      documentName:
        "german nuances, spanish nuances and french nuances / txt files",
      documentType: "txt files",
      aiResponse,
