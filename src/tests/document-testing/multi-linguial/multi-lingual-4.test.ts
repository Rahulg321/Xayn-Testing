...
let documentName = "spanish-nuances.txt";
let testCategory = "Multi Lingual Capabilities";

let prompt =
  "Provide the co-pilot with a set of Spanish legal texts that include contextual nuances and variations. Evaluate its ability to interpret the context correctly and provide accurate responses. ";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      ...
      documentName,
      documentType: "spanish legal text nuances/ text file",
      aiResponse,
