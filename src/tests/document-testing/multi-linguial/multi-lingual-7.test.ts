...
let documentName = "law_insider.docx";
let testCategory = "Multi Lingual Capabilities";

let prompt =
  "Provide a set of legal texts in English to the co-pilot and request translations into German. ";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      ...
      documentName,
      documentType: "law insider/ docx file",
      aiResponse,
