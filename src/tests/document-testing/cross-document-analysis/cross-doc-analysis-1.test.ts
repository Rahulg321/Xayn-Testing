...
let testCategory = "Cross Document Analysis";

let prompt = "Test for detecting inconsistencies across agreements";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  beforeAll(async () => {
    ...
    let documents = [
      {
        path: "./src/documents/nda-agreement.pdf",
        name: "nda-agreement.pdf",
      },
      {
        path: "./src/documents/Mutual_NonDisclosure_Agreement.pdf",
        name: "Mutual_NonDisclosure_Agreement.pdf",
      },
    ];
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      ...
      documentName: "Non Disclosure Agreement and NDA Agreement",
      documentType: "Law Party Agreements",
      aiResponse,
