...
let documentName = "partnership_agreement.pdf";
let testCategory = "Data Extraction for Custom Use";

let prompt =
  "Test for extracting critical information for external use. Extract Critical Information from this doc";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      prompt,
      testCategory,
      testName: "Custom Use Data Extraction",
      documentName,
