...
let documentName = "master_service_agreement.pdf";
let testCategory = "Audit trail and Document History";

let prompt =
  "Test for accuracy in suggesting clauses, Suggest various clauses in evaluating this agreement";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      prompt,
      testCategory,
      testName: "AI Clause Recommendation",
      documentName,
      documentType: "Master Service Agreement / pdf file",
      aiResponse,
