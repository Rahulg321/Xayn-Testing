...
let prompt =
  "What does Exemption from Registration mean under the subscription agreement?";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  beforeAll(async () => {
    ...
    const documentPath = "./src/documents/subscription_agreement.docx"; // replace with your document path
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      prompt,
      testCategory: "Legal Terminology Recognition",
      testName: "Article1",
      documentName,
