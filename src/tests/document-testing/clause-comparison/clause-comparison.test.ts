...
let prompt =
  "How would you differentiate this employment agreement from other types of agreements, such as a non-disclosure agreement or consulting agreement?";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  beforeAll(async () => {
    ...
    const documentPath = "./src/tests/documents/law_insider.docx"; // replace with your document path
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      prompt,
      testCategory: "Clause Comparison",
      testName: "Clause Comparison",
