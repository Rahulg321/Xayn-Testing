...
let testCategory = "Jurisdictional Compliance";

let prompt =
  "Upload legal documents from different jurisdictions and check noxtua abaility to recognize and evaluate it";
...
describe(`Document Testing: ${prompt}`, async () => {
  ...
  afterAll(async () => {
    await saveDocumentTestResult({
      prompt,
      testCategory,
      testName: "Jurisdictional Compliance",
      documentName:
